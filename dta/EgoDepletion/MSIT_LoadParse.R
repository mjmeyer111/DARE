...
### Read in file
msit.targetfile <- dir(pattern = 'MSIT200_EData.txt',ignore.case = TRUE)
...
msit = within(msit,{   #Calculate accuracy and trial duration within the data environment (makes the code much easier to read)
  ...
  TrialTypeNumAccOnly = ifelse(!is.na(TrialTypeNum) & is.na(TrialDur),
                               max(TrialTypeNum,na.rm = TRUE) + 2, TrialTypeNum) # recode omission errors to 4
  TrialTypeNumAccOnly = ifelse(!is.na(TrialTypeNum) & !is.na(TrialDur) & Acc == 0,
                               max(TrialTypeNum, na.rm = TRUE) + 1,
                               TrialTypeNumAccOnly) # recode comission errors to 3
})
...
msit.cast.means = data.frame(cast(msit.melt, Subject + Task ~ TrialTypeNum + Acc,
                                fun.aggregate = mean,subset = (Acc == 1)))
names(msit.cast.means)[3:4] = c('C_1_MeanRT','I_1_MeanRT')
msit.SubjectData = msit.cast.means

###### Accurate Trials (Across Trial Type)
msit.cast.means.overall = data.frame(cast(msit.melt,Subject + Task ~ Acc,
                                          fun.aggregate = mean, subset = (Acc == 1)))
...
##### RT SD by:
###### Trial Type (Accurate Only)
msit.cast.sd = data.frame(cast(msit.melt,Subject + Task  ~  TrialTypeNum + Acc,
                               fun.aggregate = sd, subset = (Acc == 1)))
names(msit.cast.sd)[3:4] = c('C_1_SDRT','I_1_SDRT')
msit.SubjectData = merge(msit.SubjectData,msit.cast.sd)
rm(msit.cast.sd)

###### Accurate Trials (Across Trial Type)
msit.cast.sd.overall=data.frame(cast(msit.melt,Subject + Task  ~  Acc,fun.
                                     aggregate = sd,subset = (Acc == 1)))
...
msit.cast.response = data.frame(cast(msit.melt,Subject + Task ~ TrialTypeNum,
                                     fun.aggregate = function(x) mean(x == 0)))
