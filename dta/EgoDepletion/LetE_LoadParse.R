...
### Read in file
LetE.targetfile <- dir(pattern = 'LetterE_EData.txt',ignore.case = TRUE)
...
print('The following subjects appear to be assigned to the wrong condition',
      quote = FALSE)
print(MisMapSubs)
...
### Accuracy Stats
#### Overall
LetE.OverallAcc = data.frame(cast(LetE.melt.DurAcc,Subject + Task ~ variable,
                                  fun.aggregate=mean,na.rm=TRUE,subset=variable == 'Acc'))
names(LetE.OverallAcc)[3] = 'Acc.Overall'
LetE.SubjectData = LetE.OverallAcc
rm('LetE.OverallAcc')

#### Accuracy by Trial Type
LetE.AccByTrialType = data.frame(cast(LetE.melt.DurAcc,Subject + Task ~ TrialType + variable,
...
#### Accuracy by Go NoGo
LetE.AccByGoNoGo = data.frame(cast(LetE.melt.DurAcc,Subject + Task ~ GoNoGo + variable,
                                   fun.aggregate = mean, na.rm = TRUE,
                                   subset = variable == 'Acc'))
...
LetE.SigDet = data.frame(cast(LetE.melt.SigDet,Subject + Task ~ variable,
                              fun.aggregate = table))
